...
function generateShipExplosion()
{
    ...
    for(i; i > -1; --i)
    {
        ...
        p.pos.setXY(ship.pos.getX() + Math.cos(p.vel.angle()) * (ship.radius * 0.8), ship.pos.getY() + Math.sin(p.vel.angle()) * (ship.radius * 0.8));
...
function generateAsteroidExplosion(asteroid)
{
    ...
    for(i; i > -1; --i)
    {
        ...
        p.vel.setLength(20 / p.radius);
        p.vel.setAngle(ship.angle + (1 - Math.random() * 2) * doublePI);
        p.pos.setXY(asteroid.pos.getX() + Math.cos(p.vel.angle()) * (asteroid.radius * 0.8), asteroid.pos.getY() + Math.sin(p.vel.angle()) * (asteroid.radius * 0.8));
